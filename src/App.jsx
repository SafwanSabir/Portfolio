...
import natureBg from './assets/nature.gif'
...
function App() {
  ...
  const [themeDragBar, setThemeDragBar] = useState(() => localStorage.getItem('barcolor') || '#008081')
  ...
  {expand: false, show: false, hide: false, focusItem: true, x: 0, y: 0, zIndex: 1,});
  const [ChessExpand, setChessExpand] = useState(
  {expand: false, show: false, hide: false, focusItem: true, x: 0, y: 0, zIndex: 1,});
  ...
  const [desktopIcon, setDesktopIcon] = useState(() => {
    ...
    if (localItems) {
      // Force remove deleted icons from existing localStorage
      parsedItems = parsedItems.filter(item => !deleteIcon.includes(item.name));

      // Ensure existing icons have their properties (like pic) synced with the latest icon.json
      ...
      // Ensure any newly added apps in icon.json (like Chess, Contact) are added to existing users
      const existingNames = parsedItems.map(i => i.name);
      filteredBaseItems.forEach(baseItem => {
        if (!existingNames.includes(baseItem.name)) {
...
function handleShowInfolderMobile(name, type) { //important handleshow for in folder
  ...
  if(loading && !login) {
    const localThemeBg = localStorage.getItem('theme') || '#008081'; 
